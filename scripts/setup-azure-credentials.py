...
def create_state_resource_group(project_name: str, location: str) -> str:
    ...
    if result.returncode == 0:
        ...
        run_command([
            ...
            '--name', rg_name,
            '--location', location,
            '--tags', f'Purpose=TerraformState', f'Project={project_name}'
        ])
        print_success(f"Created resource group: {rg_name}")
    
    return rg_name

def create_storage_accounts(project_name: str, rg_name: str, location: str, 
    ...
    for env in environments:
        ...
        run_command([
            ...
            '--tags', f'Environment={env}', 'Purpose=TerraformState', f'Project={project_name}'
        ])
...
def create_service_principal(project_name: str, subscription_id: str, 
                           sp_type: str) -> Dict[str, str]:
    ...
    for role in additional_roles:
        ...
        ], check=False)  # Don't fail if role already assigned
...
def generate_env_file(project_name: str, azure_info: Dict[str, str], 
                     terraform_sp: Dict[str, str], storage_accounts: Dict[str, str]) -> None:
...
def generate_backend_configs(project_name: str, rg_name: str, 
                           storage_accounts: Dict[str, str], environments: List[str]) -> None:
...
def main():
    ...
    try:
        ...
        storage_accounts = create_storage_accounts(args.project_name, rg_name, 
                                                 args.location, args.environments)
        terraform_sp = create_service_principal(args.project_name, 
                                               azure_info['subscription_id'], 'terraform')
        github_sp = create_service_principal(args.project_name, 
                                            azure_info['subscription_id'], 'github-actions')
        ...
        if 'sp_output' in github_sp:
            ...
            print_success("GitHub Actions credentials saved to github-actions-credentials.json")
