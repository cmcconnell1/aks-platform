    ...
    if [[ -f "$terraform_file" ]]; then
        ...
        for provider in "${required_providers[@]}"; do
            if grep -q "source.*$provider" "$terraform_file"; then
                print_success "  ✓ $provider provider is configured"
            else
                print_error "  ✗ $provider provider is missing"
                return 1
            fi
        done
        ...
        if grep -q "gavinbunney/kubectl" "$terraform_file"; then
            print_success "  ✓ kubectl provider uses correct source (gavinbunney/kubectl)"
        else
            print_error "  ✗ kubectl provider should use 'gavinbunney/kubectl' source"
            return 1
        fi
    else
