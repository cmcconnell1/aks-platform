...
class EnvironmentCredentialsSetup:
    ...
    def create_environment_service_principal(self, environment: str, 
                                           subscription_id: str) -> Dict[str, str]:
        """Create environment-specific service principal."""
        sp_name = f"{self.project_name}-{environment}-sp"
        
        print_status(f"Creating service principal for {environment} environment: {sp_name}")
        ...
        if result.stdout.strip():
            ...
            print_warning(f"Using existing service principal. You may need to reset credentials.")
    ...
    def assign_environment_roles(self, app_id: str, subscription_id: str, environment: str):
        ...
        env_roles = {
            ...
            'prod': ['Storage Account Contributor', 'Network Contributor', 'Security Admin']
        }
    ...
    def setup_environment_credentials(self, environment: str, 
                                    subscription_id: str = None) -> Dict[str, str]:
        ...
        # Create environment-specific service principal
        sp_info = self.create_environment_service_principal(environment, subscription_id)
    ...
    def generate_environment_credentials_file(self, environment: str, sp_info: Dict[str, str]):
        ...
        print_warning(f"Keep {credentials_file} secure - it contains sensitive credentials")
    ...
    def setup_github_environment_secrets(self, environment: str):
        ...
        if os.path.exists(credentials_file):
            result = self.run_command([
                ...
                '--env', environment
            ], input=open(credentials_file).read(), check=False)
...
def main():
    ...
    print_status(f"Setting up environment-specific credentials for: {', '.join(environments)}")
