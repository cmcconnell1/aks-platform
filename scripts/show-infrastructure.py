...
try:
    from azure_utils import (
        AzureHelper, print_status, print_success, print_warning, print_error,
        VirtualEnvironmentChecker, DependencyChecker
    )
...
class InfrastructureOverview:
    ...
    def get_resource_groups(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.run_command([
                'az', 'group', 'list',
                '--query', f"[?contains(name, '{self.project_name}')]",
                '--output', 'json'
            ])
    ...
    def get_storage_accounts(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.run_command([
                ...
                '--query', f"[?contains(name, '{self.project_name.replace('-', '')}')]",
                '--output', 'json'
            ])
            return json.loads(result.stdout)
        except Exception as e:
    ...
    def get_service_principals(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.run_command([
                'az', 'ad', 'sp', 'list',
                '--query', f"[?contains(displayName, '{self.project_name}')]",
                '--output', 'json'
            ])
            return json.loads(result.stdout)
        except Exception as e:
        ...
        if info["backend_config_exists"]:
            try:
                ...
                subprocess.run(['terraform', 'init', 
                              ...
                              cwd=terraform_dir, capture_output=True, check=False)
    ...
    def get_environment_costs(self, environment: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            result = self.run_command([
                ...
                '--query', f"[?contains(instanceName, '{self.project_name}-{environment}')]",
                '--output', 'json'
            ], check=False)
    ...
    def get_github_secrets(self) -> Dict[str, Any]:
        ...
        try:
            result = self.run_command(['gh', 'secret', 'list', '--json', 'name,visibility'], check=False)
            if result.returncode == 0:
    ...
    def display_environment_details(self, environment: str, include_costs: bool = False):
        ...
        if "error" in tf_info:
            ...
            print(f"Backend Config: {'YES' if tf_info['backend_config_exists'] else 'NO'}")
            ...
            if tf_info['state_info']:
                if "error" in tf_info['state_info']:
                    ...
                    print(f"Resources: {tf_info['state_info'].get('resource_count', 0)}")
                    print(f"TF Version: {tf_info['state_info'].get('terraform_version', 'Unknown')}")
        print()

        # AKS Clusters
        aks_clusters = [cluster for cluster in self.get_aks_clusters()
        ...
        env_resource_groups = [rg for rg in self.get_resource_groups()
                              if environment in rg.get('name', '')]
        ...
        if env_exists:
            ...
            if "error" in github_env_info:
                ...
                if github_env_info['secret_count'] == 0 and github_env_info['variable_count'] == 0:
        ...
        if include_costs:
            ...
            if "error" in cost_info:
                ...
                print(f"Total Cost: ${cost_info['total_cost']:.2f} {cost_info['currency']}")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Show infrastructure overview for Azure AKS Platform')
    parser.add_argument('--project-name', default='aks-platform',
                       help='Project name (default: aks-platform)')
    parser.add_argument('--environment',
                       help='Show details for specific environment only')
