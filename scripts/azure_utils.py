...
class AzureHelper:
    ...
    def get_resource_groups(self, name_filter: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            resource_groups = [rg.strip() for rg in result.stdout.split('\n') if rg.strip()]
    ...
    def get_locations(self) -> List[str]:
        ...
        try:
            ...
            return ['eastus', 'westus', 'westus2', 'centralus', 'eastus2', 'southcentralus']  # US regions fallback
...
class DependencyChecker:
    ...
    def check_python_version(min_version: Tuple[int, int] = (3, 7)) -> bool:
        ...
        if current < min_version:
            print_error(f"Python {min_version[0]}.{min_version[1]}+ required (found: {current[0]}.{current[1]})")
            return False
        return True
    ...
    def check_azure_dependencies(self) -> bool:
        ...
        dependencies = {
            ...
            'Azure Management': lambda: self.check_python_package('azure.mgmt.resource')
        }
...
def safe_run_command(command: List[str], capture_output: bool = True) -> Tuple[bool, str, str]:
