    ...
    try:
        ...
        for module_name in imports:
            try:
                ...
                available_modules[module_name] = MockModule(module_name, imported_module)
            except ImportError:
...
def analyze_script_dependencies(script_path):
    ...
    try:
        ...
        with open(script_path, 'r', encoding='utf-8') as f:
            ...
            if not (first_line.startswith('#!') and 'python' in first_line) and not script_path.endswith('.py'):
        ...
        print("This may not be a valid Python script or may have Python version compatibility issues.")
        return None, None, None
        ...
        if "'NoneType' object has no attribute 'is_package'" in str(e):
            ...
            print("This often happens with complex import patterns or missing dependencies.")
...
def print_loaded_modules(modules):
    ...
    known_thirdparty = {
        'requests', 'numpy', 'pandas', 'flask', 'django', 'boto3', 'azure',
        'google', 'aws', 'click', 'pyyaml', 'jinja2', 'sqlalchemy', 'pytest',
        'matplotlib', 'scipy', 'sklearn', 'tensorflow', 'torch', 'fastapi'
    }
    ...
    if stdlib_modules:
        ...
        for name, mod in sorted(stdlib_modules):
            globals_preview = list(mod.globalnames.keys())[:3] if mod.globalnames else []
...
def print_missing_modules(badmodules):
    ...
    for module_name in sorted(badmodules.keys()):
        if any(pkg in module_name.lower() for pkg in ['azure', 'boto', 'google', 'aws']):
...
def generate_requirements(modules, badmodules):
    ...
    for name, mod in modules.items():
        if hasattr(mod, '__file__') and mod.__file__ and 'site-packages' in mod.__file__:
    ...
    for module_name in badmodules.keys():
        ...
        if root_package in package_mappings:
            suggested_packages.add(package_mappings[root_package])
        elif any(pkg in root_package.lower() for pkg in ['azure', 'requests', 'boto', 'google']):
            suggested_packages.add(root_package)
...
def main():
    ...
    critical_missing = [m for m in badmodules.keys()
